...
public class WorldUtils {
	public static final EnumFacing[] FACES_AND_NULL = {null, EnumFacing.DOWN, EnumFacing.UP, EnumFacing.NORTH, EnumFacing.SOUTH, EnumFacing.WEST, EnumFacing.EAST};
	...
	public static void dropItem(World world, BlockPos pos, ItemStack stack) {
		if (!world.isRemote && !stack.isEmpty()) {
			EntityItem ent = new EntityItem(world, pos.getX() + world.rand.nextFloat() * 0.5F + 0.25D, pos.getY() + world.rand.nextFloat() * 0.5F + 0.25D, pos.getZ() + world.rand.nextFloat() * 0.5F + 0.25D, stack);
			ent.motionX = world.rand.nextGaussian() * 0.05D;
			ent.motionY = world.rand.nextGaussian() * 0.05D + 0.2D;
			ent.motionZ = world.rand.nextGaussian() * 0.05D;
			ent.setDefaultPickupDelay();
	...
	public static boolean pointLiesInBlock(Vec3d point, BlockPos block) {
		return point.x >= block.getX() && point.x <= block.getX() + 1 && point.y >= block.getY() && point.y <= block.getY() + 1 && point.z >= block.getZ() && point.z <= block.getZ() + 1;
	}
}
