...
import lolimods.adds.lolicore.util.math.MathUtils;
...
public class StructureMatcherCuboid implements IStructureMatcher {
	...
	public boolean testStructure(WorldBlockPos basePos, List<Vec3i> components) {
		...
		if ((volumeMin > 0 && volume < volumeMin) || (volumeMax > 0 && volume > volumeMax) || !corePos.isCorePositionValid(min, max)) {
			return false;
		}
	...
	public enum CorePosition {
		ANYWHERE {
			...
			public boolean isCorePositionValid(Vec3i min, Vec3i max) {
				return min.getX() == 0 || min.getY() == 0 || min.getZ() == 0 || max.getX() == 0 || max.getY() == 0 || max.getZ() == 0;
			}
		},
		IN_FACE {
			@Override
