...
import lolimods.adds.lolicore.util.data.serialization.AutoSerialize;
import lolimods.adds.lolicore.util.data.serialization.IDatum;
import lolimods.adds.lolicore.util.data.serialization.ISerializationProvider;
import lolimods.adds.lolicore.util.data.serialization.LambdaSerializer;
...
public class DataSerialization {
	private static final Collection<ISerializationProvider<?>> DEFAULT_SERIALIZATION_PROVIDERS = Arrays.asList(
			new LambdaSerializer<>(Integer.class, NBTTagCompound::setInteger, NBTTagCompound::getInteger, ByteUtils.Writer::writeInt, ByteUtils.Reader::readInt),
			new LambdaSerializer<>(Float.class, NBTTagCompound::setFloat, NBTTagCompound::getFloat, ByteUtils.Writer::writeFloat, ByteUtils.Reader::readFloat),
			new LambdaSerializer<>(Double.class, NBTTagCompound::setDouble, NBTTagCompound::getDouble, ByteUtils.Writer::writeDouble, ByteUtils.Reader::readDouble),
			new LambdaSerializer<>(Byte.class, NBTTagCompound::setByte, NBTTagCompound::getByte, ByteUtils.Writer::writeByte, ByteUtils.Reader::readByte),
			new LambdaSerializer<>(Short.class, NBTTagCompound::setShort, NBTTagCompound::getShort, ByteUtils.Writer::writeShort, ByteUtils.Reader::readShort),
			new LambdaSerializer<>(Long.class, NBTTagCompound::setLong, NBTTagCompound::getLong, ByteUtils.Writer::writeLong, ByteUtils.Reader::readLong),
			new LambdaSerializer<>(Boolean.class, NBTTagCompound::setBoolean, NBTTagCompound::getBoolean, ByteUtils.Writer::writeBool, ByteUtils.Reader::readBool),
			new LambdaSerializer<>(String.class, NBTTagCompound::setString, NBTTagCompound::getString, ByteUtils.Writer::writeString, ByteUtils.Reader::readString),
			new LambdaSerializer<>(NBTTagCompound.class, NBTTagCompound::setTag, NBTTagCompound::getCompoundTag, ByteUtils.Writer::writeTagCompound, ByteUtils.Reader::readTagCompound),
			new LambdaSerializer<>(ItemStack.class, (t, k, s) -> {
						NBTTagCompound i = new NBTTagCompound();
						s.writeToNBT(i);
						t.setTag(k, i);
					}, (t, k) -> new ItemStack(t.getCompoundTag(k)), ByteUtils.Writer::writeItemStack, ByteUtils.Reader::readItemStack),
			new LambdaSerializer<>(Fluid.class, (t, k, f) -> t.setString(k, f.getName()), (t, k) -> FluidRegistry.getFluid(t.getString(k)), ByteUtils.Writer::writeFluid, ByteUtils.Reader::readFluid),
			new LambdaSerializer<>(FluidStack.class, (t, k, s) -> {
						NBTTagCompound f = new NBTTagCompound();
						s.writeToNBT(f);
						t.setTag(k, f);
					}, (t, k) -> FluidStack.loadFluidStackFromNBT(t.getCompoundTag(k)), ByteUtils.Writer::writeFluidStack, ByteUtils.Reader::readFluidStack),
			new LambdaSerializer<>(BlockPos.class, (t, k, p) -> t.setTag(k, NBTUtils.serializeBlockPos(p)), (t, k) -> NBTUtils.deserializeBlockPos(t.getCompoundTag(k)), ByteUtils.Writer::writeBlockPos, ByteUtils.Reader::readBlockPos),
			new LambdaSerializer<>(WorldBlockPos.class, (t, k, p) -> t.setTag(k, NBTUtils.serializeWorldBlockPos(p)), (t, k) -> NBTUtils.deserializeWorldBlockPos(t.getCompoundTag(k)), ByteUtils.Writer::writeWorldBlockPos, ByteUtils.Reader::readWorldBlockPos),
			new LambdaSerializer<>(Vec3d.class, (t, k, v) -> t.setTag(k, NBTUtils.serializeVec3d(v)), (t, k) -> NBTUtils.deserializeVec3d(t.getCompoundTag(k)), ByteUtils.Writer::writeVec3d, ByteUtils.Reader::readVec3d),
			new LambdaSerializer<>(Vec2i.class, (t, k, v) -> t.setTag(k, NBTUtils.serializeVec2i(v)), (t, k) -> NBTUtils.deserializeVec2i(t.getCompoundTag(k)), ByteUtils.Writer::writeVec2i, ByteUtils.Reader::readVec2i),
			new LambdaSerializer<>(UUID.class, (t, k, i) -> t.setString(k, i.toString()), (t, k) -> UUID.fromString(t.getString(k)), ByteUtils.Writer::writeUuid, ByteUtils.Reader::readUuid)
	);
	...
	private static List<Field> getOrComputeClassMappings(Class<?> src) {
		...
		if (mappings == null) {
			LoliCore.LOGGER.info("Calculating serialization mappings for class: {}", src.getName());
