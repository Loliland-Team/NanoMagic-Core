package lolimods.adds.lolicore.util.nbt;
...
public class NBTUtils {
	public static ChainingTagCompound serializeBlockPos(BlockPos pos) {
		return new ChainingTagCompound().withInt("x", pos.getX()).withInt("y", pos.getY()).withInt("z", pos.getZ());
	}
	...
	public static ChainingTagCompound serializeWorldBlockPos(WorldBlockPos pos) {
		return new ChainingTagCompound().withInt("dim", pos.getDimId()).withTag("pos", serializeBlockPos(pos.getPos()));
	}
	...
	public static ChainingTagCompound serializeVec3d(Vec3d vec) {
		return new ChainingTagCompound().withDouble("x", vec.x).withDouble("y", vec.y).withDouble("z", vec.z);
	}
	...
	public static <T extends NBTBase> Collector<T, NBTTagList, NBTTagList> collectList() {
		return Collector.of(NBTTagList::new, NBTTagList::appendTag, NBTUtils::mergeLists, Collector.Characteristics.IDENTITY_FINISH);
	}
