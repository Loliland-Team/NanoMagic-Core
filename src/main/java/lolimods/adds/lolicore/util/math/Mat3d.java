...
public class Mat3d {
	...
	public Vec3d multiply(Vec3d vec) {
		return new Vec3d(vec.x * values[0] + vec.y * values[1] + vec.z * values[2], vec.x * values[3] + vec.y * values[4] + vec.z * values[5], vec.x * values[6] + vec.y * values[7] + vec.z * values[8]);
	}
}
