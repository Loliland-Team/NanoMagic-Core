...
import lolimods.adds.lolicore.util.render.RenderUtils;
import lolimods.adds.lolicore.util.render.shader.IShader;
import lolimods.adds.lolicore.util.render.shader.IShaderProgram;
import lolimods.adds.lolicore.util.render.shader.ShaderType;
import lolimods.adds.lolicore.util.render.shader.Uniform;
import lolimods.adds.lolicore.util.render.shader.UniformType;
...
public class ShaderUtils {
	...
	public static void registerReloadHook() {
		((IReloadableResourceManager) Minecraft.getMinecraft().getResourceManager()).registerReloadListener(manager -> reloadShaders());
	}
	...
	public static IShader newShader(ShaderType type, ResourceLocation resource) {
		return newShader(type, () -> {
			try {
				...
				LoliCore.LOGGER.error("Could not retrieve shader {}!", resource);
	...
	private static class ShaderProgramImpl implements IShaderProgram {
		...
		void compile() {
			...
			if (GL20.glGetProgrami(programId, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
				LoliCore.LOGGER.warn("Compilation failed for shader program! Ignoring...\n{}",
