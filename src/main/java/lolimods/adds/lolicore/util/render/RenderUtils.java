...
public class RenderUtils {
	...
	public static boolean checkOptifine() {
		if (optifined == OptifineStatus.CHECK_NEEDED) {
			...
			if (optifined == OptifineStatus.PRESENT) {
				LoliCore.LOGGER.warn("Whoops! You have Optifine installed. That might cause some weird rendering issues.");
			}
		}
	...
	public static Vec3d getInterpPos(Entity entity, float partialTicks) {
		return new Vec3d(entity.prevPosX + (entity.posX - entity.prevPosX) * partialTicks, entity.prevPosY + (entity.posY - entity.prevPosY) * partialTicks, entity.prevPosZ + (entity.posZ - entity.prevPosZ) * partialTicks);
	}
	...
	public static void renderWorldOrtho(double x, double y, double z, float scaleX, float scaleY, float angle, float u1, float v1, float u2, float v2, float lookX, float lookXZ, float lookZ, float lookYZ, float lookXY) {
		...
		for (int i = 0; i < 4; ++i) {
			vertices[i] = axial.scale(2D * vertices[i].dotProduct(axial)).add(vertices[i].scale(angX * angX - axial.dotProduct(axial))).add(axial.crossProduct(vertices[i]).scale(2F * angX));
		}
	...
	public static void renderWorldOrtho(double x, double y, double z, float scaleX, float scaleY, float angle, float u1, float v1, float u2, float v2) {
		renderWorldOrtho(x, y, z, scaleX, scaleY, angle, u1, v1, u2, v2, ActiveRenderInfo.getRotationX(), ActiveRenderInfo.getRotationXZ(), ActiveRenderInfo.getRotationZ(), ActiveRenderInfo.getRotationYZ(), ActiveRenderInfo.getRotationXY());
	}
	...
	public static void renderItemIntoGui(@Nullable EntityLivingBase owner, int x, int y, ItemStack stack, @Nullable String countText) {
		if (stack.isEmpty()) {
			return;
		}
		GlStateManager.enableDepth();
