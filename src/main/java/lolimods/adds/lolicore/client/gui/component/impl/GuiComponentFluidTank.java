...
import lolimods.adds.lolicore.util.render.FluidRenderUtils;
...
public class GuiComponentFluidTank extends GuiComponent {
	...
	public void renderTooltip(float partialTicks, int mX, int mY) {
		FluidTankInfo tank = tankGetter.get();
		if (tank.fluid != null && tank.fluid.amount > 0) {
			drawTooltip(Arrays.asList(tank.fluid.getLocalizedName(), TextFormatting.GRAY + String.format("%,d / %,d mB", tank.fluid.amount, tank.capacity)), mX, mY);
		} else {
			drawTooltip(Arrays.asList(I18n.format(NameConst.INFO_EMPTY), TextFormatting.GRAY + String.format("0 / %,d mB", tank.capacity)), mX, mY);
		}
	}
}
