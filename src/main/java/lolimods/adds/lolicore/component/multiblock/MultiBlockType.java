...
public class MultiBlockType<T extends IMultiBlockUnit<T>> {
	...
	public boolean checkStructure(MultiBlockCore<T> core) {
		...
		return structureMatcher.testStructure(basePos, StreamSupport.stream(core.spliterator(), false).map(c -> c.getUnit().getWorldPos().getPos().subtract(basePos.getPos())).collect(Collectors.toList()));
	}
